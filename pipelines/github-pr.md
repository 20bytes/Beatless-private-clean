...
- `block:maintainer-discussion-required` — repo requires prior issue discussion with a maintainer before PRs
- `block:ask-first-required` — repo asks contributors to ask first before significant/refactoring PRs
- `block:scope-too-large` — issue is too broad for one atomic autonomous PR
