...
- `block:maintainer-discussion-required` — repo policy requires the current contributor to discuss the issue with a maintainer before opening a PR, and that discussion is missing
- `block:ask-first-required` — repo policy asks contributors to ask first before significant/refactoring PRs, and this candidate is significant/refactoring without prior approval
- `block:scope-too-large` — issue is an epic, roadmap, client/app build, version-scoped multi-feature change, or other broad task that is not an atomic PR
...
- CONTRIBUTING says to comment on the issue and wait for a maintainer response before starting/opening a PR → `block:maintainer-discussion-required` unless the blob shows the current contributor and a maintainer already interacted on that issue
- CONTRIBUTING says to "ask first" before significant PRs/refactoring, and the issue is a refactor/complexity/rewrite/migration candidate → `block:ask-first-required` unless prior maintainer approval is visible
- Client/app development, roadmap/epic/tracking issues, or version-scoped enhancement titles bundling multiple changes → `block:scope-too-large`
